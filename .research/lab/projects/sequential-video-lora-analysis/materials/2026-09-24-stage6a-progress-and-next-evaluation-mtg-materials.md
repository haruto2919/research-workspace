...
このとき最初の主な評価対象は、**Projectorへ入れる前の768次元特徴（pre-projector feature）**にするのが分かりやすいと考えています。

理由は、現在はLoRAだけでなくProjectorも学習しているためです。Projector後の128次元だけを見ると、
...
と分けて観察すると、「LoRAを含むencoder側で何が変わったか」を解釈しやすくなります。
...
ただし、shuffle条件ではA3をA1より先に見せることもあるため、未来chunkを先に利用します。そのためこれは厳密なonline学習条件ではなく、**時系列順に提示する効果を調べるためのoffline control**として区別して解釈する必要があります。
